...
const exportToPDF = async (entry: HistoryEntry) => {
    ...
    const addTextWithPageBreak = (text: string, x: number, options: { fontSize: number, fontStyle: string }) => {
      doc.setFont('Times', options.fontStyle);
      doc.setFontSize(options.fontSize);
      checkNewPage(options.fontSize);
      doc.text(text, x, y);
      y += options.fontSize * 0.8;
    };
    
    // Header with music note icon
    addTextWithPageBreak(' Notes Transposer', 20, { fontSize: 16, fontStyle: 'bold' });
    y += 5;
    
    // Export date
    addTextWithPageBreak(` Exported On: ${formatDate(entry.timestamp)}`, 20, { fontSize: 14, fontStyle: 'bold' });
    
    // Transposition type
    if (entry.type === 'scale') {
      addTextWithPageBreak(' Transposition Type: Scale Change', 20, { fontSize: 14, fontStyle: 'bold' });
    } else {
      addTextWithPageBreak(' Transposition Type: Semitone Shift', 20, { fontSize: 14, fontStyle: 'bold' });
    }
    
    // Scale change (only if scale type)
    if (entry.type === 'scale') {
      addTextWithPageBreak(` Scale Change: ${entry.fromScale} -> ${entry.toScale}`, 20, { fontSize: 14, fontStyle: 'bold' });
    }
    
    // Shift amount
    const shiftText = entry.semitones >= 0 ? `+${entry.semitones}` : `${entry.semitones}`;
    addTextWithPageBreak(` Semitone Shift: ${shiftText} Semitones`, 20, { fontSize: 14, fontStyle: 'bold' });
    y += 10;
    ...
    doc.setFontSize(14);
    entry.originalNotes.forEach((line) => {
      checkNewPage(14);
      doc.text(`  ${line}`, 20, y);
      y += 6;
    });
    ...
    addTextWithPageBreak(' Transposed Notes:', 20, { fontSize: 14, fontStyle: 'bold' });
    
    // Add transposed notes with proper spacing and page break handling
    doc.setFont('Times', 'normal');
    doc.setFontSize(14);
    entry.transposedNotes.forEach((line) => {
      checkNewPage(14);
      doc.text(`  ${line}`, 20, y);
      y += 6;
    });
    ...
    // Footer message
    checkNewPage(30); // Make sure footer fits on current page
    doc.setFont('Times', 'normal');
    doc.setFontSize(14);
    doc.text('Keep practicing and composing beautiful melodies!', 20, y);
    y += 10;
  ...
  const exportToDocx = async (entry: HistoryEntry) => {
    ...
    paragraphs.push(
      new Paragraph({
        children: [new TextRun({ 
          text: ` Exported On: ${formatDate(entry.timestamp)}`, 
          font: 'Times New Roman',
          bold: true,
          size: 28
        })],
    ...
    if (entry.type === 'scale') {
      paragraphs.push(
        new Paragraph({
          children: [new TextRun({ 
            text: ' Transposition Type: Scale Change', 
            font: 'Times New Roman',
            bold: true,
            size: 28
          })],
      ...
      paragraphs.push(
        new Paragraph({
          children: [new TextRun({ 
            text: ' Transposition Type: Semitone Shift', 
            font: 'Times New Roman',
            bold: true,
            size: 28
          })],
    ...
    paragraphs.push(
      new Paragraph({
        children: [new TextRun({ 
          text: ` Semitone Shift : ${shiftText} Semitones`, 
          font: 'Times New Roman',
          bold: true,
          size: 28
        })],
    ...
    // Add original notes with proper spacing
    entry.originalNotes.forEach((line) => {
      paragraphs.push(
        new Paragraph({
          children: [new TextRun({ 
    ...
    // Add transposed notes with proper spacing
    entry.transposedNotes.forEach((line) => {
      paragraphs.push(
        new Paragraph({
          children: [new TextRun({ 
    ...
    paragraphs.push(
      new Paragraph({
        children: [new TextRun({ 
          text: 'Keep practicing and composing beautiful melodies!', 
          font: 'Times New Roman',
          size: 28
        })],
        spacing: { after: 120 }
    ...
    paragraphs.push(
      new Paragraph({
        children: [new TextRun({ 
          text: '— Generated by Notes Transposer', 
          font: 'Times New Roman',
          size: 28
        })]
      })
